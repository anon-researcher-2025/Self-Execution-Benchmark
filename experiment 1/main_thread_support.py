...
'''
this script is a faster way to run the main.py script, using threading to speed up the process.
'''
...
# Prepare output file
output_file = openrouter_models[index].replace("/", "-").replace(":","-") + ".csv"

# Select model type
use_openai = False  # Set to True to use OpenAI, False to use OpenRouter
...
def process_target_word(item):
    """Process a single target word and return the results"""
    target_word, taboo_words = item
    ...
    return [target_word, predicted_data, actual_associated_words, final_score, count_words, 
            missed_predictions, predicted_true, success_out_of_true, success_out_of_false]
...
def main():
    ...
    max_workers = 10  # Adjust this number based on your system/API rate limits
    results = []
    print("working on the model " + openrouter_models[index])
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit tasks
        future_to_word = {executor.submit(process_target_word, item): item[0] for item in dataset_items}
        
        # Collect results as they complete
        for future in tqdm(concurrent.futures.as_completed(future_to_word), total=len(dataset_items)):
    ...
    with open(output_file, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["Word", "Predictions", "Generated Words", "Final Score (association detection accuracy)", 
                         "Count Words", "If Only False Predictions", "Predicted True Count", 
                         "Success Out of True", "Success Out of False"])
